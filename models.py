...
class Generator(nn.Module):
    ...
    def forward(self, x, s, masks=None, F0=None, training=False):
        ...
        bins = x.size(-2)
        batch = x.size(0)
        x = torch.reshape(x, (batch, frame, bins))
        for attn_layer in self.attention_layers:
            x = attn_layer(x)
        x = torch.reshape(x, (batch, channel, bins , frame))"""
...
class Discriminator(nn.Module):
    def __init__(self, dim_in=48, num_domains=2, max_conv_dim=384, repeat_num=4):
        ...
        self.aux_cls = Discriminator2d(dim_in= dim_in, num_domains= 10,
                                  max_conv_dim=max_conv_dim, repeat_num=repeat_num )
...
def build_model(args, F0_model, ASR_model):
    ...
    model_params = torch.load(args.Emotion_encoder_path, map_location='cpu')['model']['style_encoder']
    emotion_encoder.load_state_dict(model_params)
