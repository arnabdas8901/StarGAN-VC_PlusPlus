...
from Utils.EMORECOG.losses import compute_classification_loss
        ...
        for train_steps_per_epoch, batch in enumerate(tqdm(self.train_dataloader, desc="[train]"), 1):
            ...
            if scaler is not None:
                with torch.cuda.amp.autocast():
                    c_loss, c_losses = compute_classification_loss(self.model, self.args, mel, intensity, label )
                scaler.scale(c_loss).backward()
            else:
                c_loss, c_losses = compute_classification_loss(self.model, self.args, mel, intensity, label )
                c_loss.backward()

            self.optimizer.step('classifier', scaler=scaler)



            #compute moving average of network parameters
            self.moving_average(self.model.classifier, self.model_ema.classifier, beta=0.999)
