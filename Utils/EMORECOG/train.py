...
from Utils.EMORECOG.trainer import Trainer
...
@click.command()
@click.option('-p', '--config_path', default='/project/ardas/StarGAN_v2/Utils/EMORECOG/config.yml', type=str)
...
def main(config_path):
    ...
    # build model
    model, model_ema = build_model(F0_model, ASR_model)
    ...
    for _ in range(1, epochs+1):
        ...
        logger.info(txt)
        if results["eval/classification_loss"] < eval_loss:
            text = "Evalulation loss " +  str(results["eval/classification_loss"]) + " is better than " + str(eval_loss) + ". Hence saving the model."
            logger.info(text)
            trainer.save_checkpoint(osp.join(log_dir, 'epoch_%05d.pth' % epoch))
            eval_loss = results["eval/classification_loss"]
...
def get_data_path_list(train_path, val_path):
    ...
    #     train_path = "/project/sughosh/dataset/vctkall-voxceleb-24k/train_list.txt"
    # if val_path is None:
    #     val_path = "/project/sughosh/dataset/vctkall-voxceleb-24k/val_list.txt"
