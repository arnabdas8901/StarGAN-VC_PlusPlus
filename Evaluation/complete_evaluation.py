...
if __name__ == '__main__':
    ...
    emotion_encoder_path = "/StarGAN_v2/epoch_00194.pth"
    ...
    sv_root_path = '/Evaluation/Objective/Eval_results/'
    sv_path = os.path.join(sv_root_path)
    for emo in EMOTIONS:
        os.makedirs(os.path.join(sv_root_path, emo), exist_ok=True)
    ##chnge here
    complete_model_path = "/experiments/stargan-v2/VCTK10_ESD10_demo_alt_with_pitch/epoch_00060.pth"
    model = 'demo_alt_withPitch_epoch60_objective'
    ### end change here

    whisper_model = whisper.load_model("medium.en")
    starganv2 = get_model(F0_model, complete_model_path)
    vctk_csv = "/exp/IS2023/Evaluation/Objective/test/final/objective_meta_vctk_vctk_5emo_svm.csv"
    esd_csv = "/exp/IS2023/Evaluation/Objective/test/final/objective_esd_to_esd_5emo_svm.csv"
    ...
    with open('/Evaluation/Objective/objective_' + model + '.csv', mode='w') as csvf:
        ...
        for pos in range(df_esd_vctk.shape[0]):
            ...
            if ESD in src_spch:
                src_spch = src_spch.replace("/dataset/ESD/EmotionalSpeechDataset", "/dataset/ESD/Emotional Speech Dataset (ESD)")
            else:
                src_spch = src_spch.replace("/dataset/VCTK-Original", "/datasets/vctk-original")
