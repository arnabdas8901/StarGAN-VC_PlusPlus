...
class Generator(nn.Module):
    ...
    def forward(self, x, s, masks=None, F0=None, training=False):
        ...
        #x = self.conv11_x(x)
        """residual = x
        bsz, chn, feat, seq = x.size(0), x.size(1), x.size(2), x.size(3)
        seq_pad = seq % 16
        if seq_pad > 0:
            seq_pad = 16 - seq_pad
            x = nn.functional.pad(x, (0, seq_pad), mode='constant', value=0)


        x = x.reshape(bsz*(x.size(-1)//4), 4, chn*feat)
        for attn_layer in self.attention_layers_l1:
            x = attn_layer(x)
        x = x.reshape(bsz, chn, feat, -1)
        x = self.proj_x_l1(x)
        x = self.x_norm(x)

        x = x.reshape(bsz * (x.size(-1)//8), 8, chn * feat)
        for attn_layer in self.attention_layers_l2:
            x = attn_layer(x)
        x = x.reshape(bsz, chn, feat, -1)
        x = self.proj_x_l2(x)
        x = self.x_norm(x)

        x = x.reshape(bsz * (x.size(-1)//16), 16, chn * feat)
        for attn_layer in self.attention_layers_l3:
            x = attn_layer(x)
        x = x.reshape(bsz, chn, feat, -1)
        x = self.proj_x_l3(x)
        ...
        if F0 is not None:
            ...
            channel, feature=  F0.size(1), F0.size(2)

            F0_view = F0.view(F0.size(0), F0.size(-1), channel*feature)

            """h_0_l1 = torch.randn(2, F0.size(0) , int(self.F0_channel / 4) * feature)
            F0_view_l1, _ = self.F0_GRU_level1(F0_view, h_0_l1.to(F0_view.device))
            F0_view_l1 = F0_view_l1.reshape(F0_view.size(0), channel, feature, F0_view.size(1))
            F0_view_l1 = self.F0_Norm(F0_view_l1)

            h_0_l2 = torch.randn(4, F0.size(0), int(self.F0_channel / 4) * feature)
            F0_view_l2, _ = self.F0_GRU_level2(F0_view, h_0_l2.to(F0_view.device))
            F0_view_l2 = F0_view_l2.reshape(F0_view.size(0), channel, feature, F0_view.size(1))
            F0_view_l2 = self.F0_Norm(F0_view_l2)"""

            h_0_l3 = torch.randn(6, F0.size(0), int(self.F0_channel / 4) * feature)
            F0_view_l3, _ = self.F0_GRU_level3(F0_view, h_0_l3.to(F0_view.device))
            F0_view_l3 = F0_view_l3.reshape(F0_view.size(0), channel, feature, F0_view.size(1))
            F0_view_l3 = F0_view_l3 + F0
            F0_view_l3 = self.F0_Norm(F0_view_l3)


            #x = torch.cat([x,F0_view_l1,F0_view_l2,F0_view_l3], axis=1)
            #x = torch.cat([x, F0_view_l3], axis=1)
...
def build_model(args, F0_model, ASR_model):
    ...
    """#Phoneme segment model
    ckpt = torch.load(args.phon_seg_model_path, map_location=lambda storage, loc: storage)
    hp = Namespace(**dict(ckpt["hparams"]))
    phon_segmenter = NextFrameClassifier(hp)
    weights = ckpt["state_dict"]
    weights = {k.replace("NFC.", ""): v for k, v in weights.items()}
    phon_segmenter.load_state_dict(weights)"""

    #Emotion encoder pretrained model from ESD dataset
    #emotion_encoder = StyleEncoder(args.dim_in, args.style_dim, args.num_emotion_domain, args.max_conv_dim)
